...
class Spec extends SpecBase {
  ...
  test(eval("val f = (x: Unit, y: String, z: Boolean) => x; f"),
      "<function>: (Unit, String, Boolean) => Unit")
  test(eval("val f = (x: Unit, y: String, z: Boolean) => y; f"),
      "<function>: (Unit, String, Boolean) => String")
  test(eval("val f = (x: Unit, y: String, z: Boolean) => z; f"),
  ...
  test(eval("lazy val x: Number = y + 1; lazy val y: Number = x + 1; true"),
      "true: Boolean")
  test(eval("lazy val x: Boolean = if (x) x else x; \"abc\""),
      "\"abc\": String")
  // type mismatch: Boolean != Number
  ...
  // arity mismatch: 2 != 1
  testExc(eval("def f[T, U](x: T): T = x; f[Number](42)"), weight = 3)
  // arity mismatch: 1 != 2
  testExc(eval("def f[T](x: T): T = x; f[Number](42, 43)"), weight = 3)
  // unknown type: U
  testExc(eval("def f[T, U](x: T): T = x; f[Number, U](42)"), weight = 3)
  // unknown type: U
  testExc(eval("def f[T](x: T): U = X(); enum U {case X()}; f"), weight = 3)
  test(eval("enum T { case X() }; enum U { case X() }; val x = X(); 1"),
      "1: Number", weight = 3)
  // already defined type: T
  testExc(eval("enum T[V] { case X() }; enum U[T] { case X() }; 1"), weight = 3)
  // unknown type: U
  testExc(eval("enum T { case X(n: U) }; 1"), weight = 3)
  // type mismatch: Boolean != Number
  testExc(eval(
          shape +
            "Square(10) match { case Square(n) => true; case Rect(w, h) => 1 }"
      ), weight = 3)
  val expr14 = """
    enum T {
      case X()
  ...
  val expr15 = """
    ...
    f[U](X())
  """
  // not a variant type: T
  testExc(eval(expr15), weight = 3)
  ...
  val beHelper = """
    ...
    def countLiterals(expr: BE): Number = {
      expr match {
        case True()     => 1
        case False()    => 1
    ...
    def max(l: Number, r: Number): Number = {
      if(l < r) r else l
    }
    def depth(expr: BE): Number = {
      expr match {
        case True()     => 0
        case False()    => 0
        case And(l, r)  => 1 + (max(depth(l), depth(r)))
        case Or(l, r)   => 1 + (max(depth(l), depth(r)))
        case Not(e)     => 1 + depth(e)
      }
    }
    ...
    val be1 = And(Not(And(Or(False(), Not(True())), False())), And(True(), Not(False())))
    val be2 = Not(Not(Not(And(Not(Not(Not(True()))), Not(Not(Not(False())))))))
  """
  test(eval(beHelper + "countLiterals(be1)"), "5: Number", weight = 2)
  test(eval(beHelper + "countLiterals(be2)"), "2: Number", weight = 2)
  test(eval(beHelper + "countNots(be1)"), "3: Number", weight = 2)
  test(eval(beHelper + "countNots(be2)"), "9: Number", weight = 2)
  test(eval(beHelper + "depth(be1)"), "5: Number", weight = 2)
  test(eval(beHelper + "depth(be2)"), "7: Number", weight = 2)
  test(eval(beHelper + "getString(be1)"),
      "\"(!((false | !true) & false) & (true & !false))\": String", weight = 2)
  test(eval(beHelper + "getString(be2)"), "\"!!!(!!!true & !!!false)\": String",
      weight = 2)
  test(eval(beHelper + "eval(be1)"), "true: Boolean", weight = 2)
  test(eval(beHelper + "eval(be2)"), "true: Boolean", weight = 2)
