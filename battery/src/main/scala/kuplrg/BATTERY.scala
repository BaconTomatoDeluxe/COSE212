...
enum RecDef:
  ...
  case LazyVal(
      name: String, ty: Type, init: Expr
  )
  // recursive function
  case RecFun(
      name: String, tvars: List[String], params: List[Param], rty: Type,
      body: Expr
  )
  // polymorphic algebraic data type
  case TypeDef(
      name: String, tvars: List[String], varts: List[Variant]
  )
...
// type environments
case class TypeEnv(
    vars: Map[String, Type] = Map(), tys: Map[String, TypeInfo] = Map()
) {
  import TypeInfo.*
...
object Parser extends RegexParsers with PackratParsers {
  ...
  lazy val expr: P[Expr] =
    ...
    lazy val e1: P[Expr] = rep1(recDef <~ opt(";")) ~ e2 ^^ { case ds ~ e =>
      dupCheck(ds.flatMap {
    ...
    lazy val e5: P[Expr] = e6 ~ rep(("==" | "!=") ~ e5) ^^ { case e ~ es =>
      es.foldLeft(e) {
        case (l, "==" ~ r) => EEq(l, r)
        case (l, _ ~ r)    => ENe(l, r)
      }
    ...
    lazy val e7: P[Expr] = e8 ~ rep(("++" | "+" | "-") ~ e8) ^^ { case e ~ es =>
      es.foldLeft(e) {
        case (l, "+" ~ r) => EAdd(l, r)
        case (l, "-" ~ r) => ESub(l, r)
    ...
    lazy val e8: P[Expr] = e9 ~ rep(("*" | "/" | "%") ~ e9) ^^ { case e ~ es =>
      es.foldLeft(e) {
        case (l, "*" ~ r) => EMul(l, r)
        case (l, "/" ~ r) => EDiv(l, r)
        case (l, _ ~ r)   => EMod(l, r)
    ...
    lazy val e10: P[Expr] = e11 ~ rep(
      ...
      as.foldLeft(f) { case (f, tsOpt ~ as) =>
        EApp(f, tsOpt.getOrElse(Nil), as)
      }
    }
    lazy val e11: P[Expr] = (
        "(" ~ ")" ^^^ EUnit |
  ...
  private lazy val tvars: P[List[String]] =
    opt("[" ~> rep1sep(id, ",") <~ "]") ^^ { case xs =>
      dupCheck(xs.getOrElse(Nil), "type variable")
    }
  ...
  private lazy val ty: P[Type] =
    ...
    lazy val t1: P[Type] = (
        "(" ~> t0 <~ ")" |
          ...
          "Boolean" ^^^ BoolT |
          "String" ^^^ StrT |
          id ~ opt("[" ~> repsep(t0, ",") <~ "]") ^^ { case x ~ tsOpt =>
            IdT(x, tsOpt.getOrElse(Nil))
          }
    )
    t0
  ...
  private def dupCheck[T](
      names: List[T], kind: String, f: T => String = (x: T) => x.toString
  ): List[T] =
