...
class Spec extends SpecBase {
  ...
  // newly added tests with weighted points
  testExc(eval("1 :: 2"), "", weight = 2)
  testExc(eval("2 :: true"), "", weight = 2)

  test(eval("List(1) == List(1, true)"), "false", weight = 2)
  test(eval("List(1, 2, 3) == List(2, true)"), "false", weight = 2)
  testExc(eval("List(1, 2) == List(true)"), "", weight = 2)
  testExc(eval("List(1, 2) == List(1, true)"), "", weight = 2)

  test(eval("List().map(42)"), "List()", weight = 2)
  test(eval("List().flatMap(42)"), "List()", weight = 2)
  test(eval("List().filter(42)"), "List()", weight = 2)

  testExc(eval("List().map(x)"), "", weight = 2)
  testExc(eval("List().flatMap(x)"), "", weight = 2)
  testExc(eval("List().filter(x)"), "", weight = 2)

  testExc(eval("List(1, 2, 3).map(List())"), "", weight = 2)
  testExc(eval("List(1, 2, 3).flatMap(true)"), "", weight = 2)
  testExc(eval("List(1, 2, 3).filter(42)"), "", weight = 2)

  testExc(eval("List(1).flatMap(x => x)"), "", weight = 2)
  testExc(eval("List(1).flatMap(x => true)"), "", weight = 2)

  test(eval("(1, 2) == (3, true)"), "false", weight = 2)
  test(eval("(1, 2, 3) == (2, 3, true)"), "false", weight = 2)
  testExc(eval("(1, 2, 3) == (true)"), "", weight = 2)
  testExc(eval("(1, 2, 3) == (1, 2, true)"), "", weight = 2)

  testExc(eval("(1, x)._1"), "", weight = 2)
  testExc(eval("(1, 2, x)._1"), "", weight = 2)

  test(eval("(1, true)._2"), "true", weight = 2)
  test(eval("(1, 2, 3)._3"), "3", weight = 2)

  // perform fuzz testing only if all manual tests passed
  def fuzzTest: Unit =
    if (getFailCount == 0)
      val fuzzer = Fuzzer(eval)
      val time = 60000 // 1 minutes
      fuzzer.check(time)
    else
      error("already failed some tests")

  check(fuzzTest, weight = 25)
