...
class Spec extends SpecBase {
  ...
  test(eval("var x = 0; while(true) { if (x > 1) break else x += 1 }; x"), "2",
      weight = 3)
  test(eval("var x = 0; while(x < 3) { x += 1; continue; return 0 }; x"), "3",
      weight = 3)
  ...
  test(eval("var f = x => throw x + 1; try f(1) * 5 catch (e) e"), "2",
      weight = 3)
  test(eval("x =>* x"), "<generator>", weight = 3)
  test(eval("(x =>* x)(42)"), "<iterator>", weight = 3)
  test(eval("(x =>* x)(42).next()"), "{ value: 42, done: true }", weight = 3)
  test(eval("(x =>* yield x)(42).next()"), "{ value: 42, done: false }",
      weight = 3)
  test(eval("(x =>* x)(42).next().value"), "42", weight = 3)
  test(eval("(x =>* x)(42).next().done"), "true", weight = 3)
  test(eval("(x =>* { return 1; 2 })().next()"), "{ value: 1, done: true }",
      weight = 3)
  test(eval("(x =>* { throw 42; 2 })().next()"), "{ value: 42, done: true }",
      weight = 3)
  test(eval("(x =>* (yield x + 1) * 3)(2).next()"), "{ value: 3, done: false }",
      weight = 3)
  test(eval("var i = (x =>* yield(yield 1))(); i.next(); i.next(7).value"), "7",
      weight = 3)
  ...
  def pass = (0 until 50).forall { case k =>
    eval(squareSumExpr(k / 2 + 1, k)) == squareSum(k / 2 + 1, k).toString
  }
  test(passAll && pass, true, weight = 40)

  // new tests for squareSumExpr
  def newPass = (-10 to 10).forall { case i =>
    (-10 to 10).forall { case j =>
      eval(squareSumExpr(i, j)) == squareSum(i, j).toString
    }
  }
  ...
  check(fuzzTest, weight = 70, timeLimit = 65)
   */
}
